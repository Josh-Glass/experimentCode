var classificationTrain = {
    ...
    start: function(params, next) {
        ...
        space = oCanvas.create({
            canvas: classificationTrain.initCanvas(),
            background: 'rgba(255,255,255)',
            // origin: { x: "center", y: "center" },
        })
        classificationTrain.initElements(params)
        classificationTrain.initDebugWindow(params)
        ...
        trial = 0
        block = 0
        block_order = classificationTrain.generateBlockOrder(params['objects'], params['labels']) // <-- initialize the first block order (you'll do this every block)

        correctCategory = block_order[trial][1]
        classificationTrain.initStim(block_order[trial][0])
        ...
        continueBtn.bind('click tap', function () {
            ...
            if (trial == block_order.length) {
                ...
                if (block == params['n_blocks']) {
                    ...
                    block_order = classificationTrain.generateBlockOrder(params['objects'], params['labels'])
                }
            }
        ...
        console.log('started classificationTrain')
    },
    ...
    generateBlockOrder: function (objects, labels) {
        let block = []
        idx = classificationTrain.shuffle([...Array(labels.length).keys()]) // from ben @ https://stackoverflow.com/a/10050831
