var classificationTest = {
    ...
    start: function(params, next) {
        ...
        space = oCanvas.create({
            canvas: classificationTest.initCanvas(),
            background: 'rgba(255,255,255)',
            // origin: { x: "center", y: "center" },
        })
        classificationTest.initElements(params)
        classificationTest.initDebugWindow(params)
        ...
        trial = 0
        block = 0
        block_order = classificationTest.generateBlockOrder(params['objects'], params['labels']) // <-- initialize the first block order (you'll do this every block)

        correctCategory = block_order[trial][1]
        classificationTest.initStim(block_order[trial][0])
        ...
        for (category of params['categories']) {
            responseBtns[category].btn.bind('click tap', function () {
                ...
                if (acc == true) {
                    feedbackTxt.text = 'Your response has been recorded'
                } else {
                    feedbackTxt.text = 'Your response has been recorded'
                }
        ...
        continueBtn.bind('click tap', function () {
            ...
            if (trial == block_order.length) {
                ...
                if (block == params['n_blocks']) {
                    ...
                    block_order = classificationTest.generateBlockOrder(params['objects'], params['labels'])
                }
            }
        ...
        console.log('started classificationTest')
    },
    ...
    generateBlockOrder: function (objects, labels) {
        let block = []
        idx = classificationTest.shuffle([...Array(labels.length).keys()]) // from ben @ https://stackoverflow.com/a/10050831
